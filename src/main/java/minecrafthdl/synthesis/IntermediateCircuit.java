...
public class IntermediateCircuit {
    ...
    public void loadGraph(Graph graph) {
        ...
        for (Vertex v : graph.getVertices()) {
            if (v.getType() == VertexType.INPUT) {
                ...
                if (f.func_type == FunctionType.HIGH || f.func_type == FunctionType.LOW) {
                    in_process.add(v);
                }
            }
        }
    ...
    public void buildGates() {
        if (this.vertex_layers.size() == 0) {
            throw new MHDLException("Must load graph before building gates");
        }
    ...
    public Circuit genCircuit() {
        if (this.gate_layers.size() == 0) {
            throw new MHDLException("Must build gates before generating final circuit");
        }
        ...
        for (ArrayList<Gate> layer : this.gate_layers) {
            ...
            for (Circuit c : layer) {
                ...
                if (c.getSizeY() > this_size_y) {
                    this_size_y = c.getSizeY();
                }
                if (c.getSizeZ() > this_size_z) {
                    this_size_z = c.getSizeZ();
                }
            }
                ...
                if (g.getSizeZ() - 1 < layers_size_z[i] && !Circuit.TEST) {
                    for (int z = g.getSizeZ(); z < layers_size_z[i]; z++) {
                        if (z == layers_size_z[i] - 1) {
                            ...
                            circuit.setBlock(x_offset, 0, z_offset + z, Blocks.REDSTONE_WIRE.defaultBlockState());
                        }
                    }
                }
    ...
    private Gate genGate(Vertex v) {
        if (getFunctionType(v) == FunctionType.AND) {
            ...
            return Circuit.TEST ? TestLogicGates.XOR() : LogicGates.XOR();
            ...
            return Circuit.TEST ? TestLogicGates.LOW() : LogicGates.LOW();
        } else if (getFunctionType(v) == FunctionType.D_LATCH) {
            return Circuit.TEST ? TestLogicGates.D_LATCH() : LogicGates.D_LATCH();
